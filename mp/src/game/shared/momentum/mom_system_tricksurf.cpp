...
void CTrickSystem::OnTrickFailed(CTrickAttempt *pAttempt)
{
    ...
    if (pAttempt->GetTrick()->GetID() == m_iTrackedTrick && pAttempt->GetCurrentStep() != 0)
    {
        OnTrickTrackTerminate();
    }
}
...
void CTrickSystem::UpdateTrackedTrickTriggers()
{
    ...
    for (int i = 0; i < iCurrentStep + 2 && i < iTrickStepCount; i++)
    {
        ...
        if (i == iCurrentStep)
        {
            ...
            {
                // MOM_TODO loop through optionals until at least one required
                pStepTrigger->m_iDrawState = pStep->IsOptional() ? TRICK_DRAW_OPTIONAL : TRICK_DRAW_REQUIRED;
            }
        }
        else
        {
...
void CTrickSystem::TeleportToTrick(int iTrickID)
{
    ...
    pPlayer->ManualTeleport(&pTrigger->WorldSpaceCenter(), nullptr, &vec3_origin);
}
...
#endif
    return CFmtStr("%s/%s.tricks", ZONE_FOLDER, pMapName);
}
